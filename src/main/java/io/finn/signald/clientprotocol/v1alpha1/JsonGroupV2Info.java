...
public class JsonGroupV2Info {
  ...
  public JsonGroupV2Info(io.finn.signald.clientprotocol.v1.JsonGroupV2Info g) {
    id = g.id;
    ...
    if (g.pendingMembers != null) {
      pendingMembers = new ArrayList<>();
      for (JsonAddress m : g.pendingMembers) {
    ...
    if (g.requestingMembers != null) {
      requestingMembers = new ArrayList<>();
      for (JsonAddress m : g.requestingMembers) {

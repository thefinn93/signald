...
public class SecurityProvider extends Provider {
  ...
  public static class DefaultRandom extends SecureRandomSpi {
    ...
    protected void engineSetSeed(byte[] bytes) { random.setSeed(bytes); }

    protected void engineNextBytes(byte[] bytes) { random.nextBytes(bytes); }

    protected byte[] engineGenerateSeed(int numBytes) { return random.generateSeed(numBytes); }
  }
}
